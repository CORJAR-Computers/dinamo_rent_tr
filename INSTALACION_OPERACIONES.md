# Instalación de Dinamo Rent ERP — v1.2.2 (estable)

> Guía para el equipo de operaciones. **Última versión estable: v1.2.2** — ronda de QA:
> fechas locales en toda la app (en UTC-5, tras las 7 PM se proponía la fecha de
> mañana; la más crítica, la devolución real del cierre de rentas), doble cobro
> eliminado en extensiones de renta y salvaguardas al restaurar la BD (copia
> pre_restore + rechazo de backups vacíos). Con el **auto-update** activo desde la
> v1.0.14 (la app detecta y ofrece instalar las versiones nuevas al arrancar, ver
> sección 6) y las features previas (cierre de renta completo, contrato con desglose
> de costos, costo de lavado en reservas, atomicidad en mantenimiento, auditoría
> completa, documento en 2 hojas). Construida y validada por CI.
>
...
Página de la release: <https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/tag/v1.2.2>

| Instalador | Enlace directo | Tamaño | Uso |
|---|---|---|---|
| **NSIS** (`DinamoRent_1.2.2_x64-setup.exe`) | <https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.2.2/DinamoRent_1.2.2_x64-setup.exe> | ~21 MB | **Recomendado** — instalación asistida con atajo de escritorio |
| **MSI** (`DinamoRent_1.2.2_x64_en-US.msi`) | <https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.2.2/DinamoRent_1.2.2_x64_en-US.msi> | ~32 MB | Despliegue empresarial / GPO |

> ⚠️ **No usar la v1.0.0** (descontinuada): falla en equipos sin BD previa. Si un equipo
> ya la tiene instalada **con datos**, no hay que desinstalar — la v1.2.2 abre la BD
...
1. Ejecutar `DinamoRent_1.2.2_x64-setup.exe` como usuario normal.
...
DinamoRent_1.2.2_x64-setup.exe /S
```

### Opción C — Silenciosa (MSI, para GPO/Intune)
```powershell
msiexec /i DinamoRent_1.2.2_x64_en-US.msi /qn /norestart
```
...
- **Actualizar desde v1.0.0**: instalar la v1.2.2 encima. Idempotente, sin pérdida de datos.
- **Actualizar desde v1.0.2**: instalar la v1.2.2 encima (transición al auto-update).
