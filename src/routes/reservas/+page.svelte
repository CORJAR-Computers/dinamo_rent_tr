<script lang="ts">
	...
	async function cargar() {
		...
		loading = true;
		try {
			reservas = await reservaApi.listar(
				sid(),
	...
	// ── Crear renta desde una reserva ──
	// Navega a /rentas?desdeReserva=<id>: la página de rentas abre el formulario
	// precargado con cliente, vehículo, fechas y tarifas de esta reserva.
	function crearRentaDesdeReserva(r: Reserva) {
		goto(`/rentas?desdeReserva=${r.id}`);
	}
		...
		>
			{#snippet children(col, item)}
				...
				{:else if col.key === 'retorno'}
					<div class="whitespace-nowrap">
						<p class="text-text-primary tabular-nums">
							{formatDate(r.fechaRetorno)}
							<span class="text-text-secondary">{fmtHora(r.horaRetorno)}</span>
						</p>
...
<Modal
	...
	subtitle={editando
		...
		: 'Registra una reserva para un cliente.'}
			...
			<div class="flex-1 min-w-0 overflow-y-auto din-scroll px-5 py-4">
				...
				<div class="grid grid-cols-2 gap-x-3 mb-3">
					...
					<FormField label="Lugar de recogida" dense>
						<input
							class="input"
							placeholder="Ej: Aeropuerto, oficina..."
				...
				<div class="px-4 py-3 border-b border-border">
					...
					>
						...
						<p class="text-[10px] opacity-80 mt-0.5">
							{form.diasCalculados} día{form.diasCalculados === 1 ? '' : 's'}{form.horasExtras > 0
								? ` + ${form.horasExtras}h`
								: ''}
						</p>
					</div>
