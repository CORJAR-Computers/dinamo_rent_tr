...
<div class="min-h-screen flex items-center justify-center bg-fondo relative overflow-hidden">
	...
	<div class="absolute inset-0 pointer-events-none" aria-hidden="true">
		<div
			class="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-primary/10 blur-3xl"
		></div>
		<div
			class="absolute -bottom-40 -left-40 w-[400px] h-[400px] rounded-full bg-primary-focus/10 blur-3xl"
		></div>
	</div>
	...
	<div class="relative w-full max-w-md px-4">
		...
		<div class="flex flex-col items-center mb-8">
			<div
				class="w-28 h-28 mb-4 rounded-3xl bg-white shadow-lg shadow-primary/20 ring-1 ring-border overflow-hidden"
			>
				<img
					src={empresa.logoSrc}
			...
			<h1 class="text-2xl font-bold text-text-primary tracking-tight">
				{empresa.nombreMostrar}{#if !empresa.nombre}
					ERP{/if}
			</h1>
			<p class="text-sm text-text-secondary mt-1">
				Sistema de gestión de flota y renta de vehículos
			</p>
		</div>
		...
		<div class="card p-8 shadow-xl">
			...
			>
				...
				{#if locked && loginStatus}
					<div
						class="mt-3 mb-1 rounded-lg bg-alerta/10 border border-alerta/30 px-3 py-2.5 text-sm text-alerta flex items-center gap-2"
						role="alert"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
						...
						>
						<span>
							Cuenta bloqueada. Intenta nuevamente en {Math.ceil(
								loginStatus.lockoutRemainingSeconds / 60
							)} min.
						</span>
					</div>
				{/if}
				...
				<button type="submit" class="btn-primary w-full mt-5 py-2.5" disabled={loading || locked}>
					{#if loading}
						<svg
			...
			<footer class="mt-6 pt-5 border-t border-border text-center">
				<p class="text-xs text-text-secondary">
					...
					{empresa.nombreMostrar}{#if app.version}
						· v{app.version}{/if}
				</p>
			</footer>
		</div>
