...
<div class="space-y-5">
	...
	<div class="flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex items-center gap-2">
			<label
				class="inline-flex items-center gap-2 text-sm text-text-primary cursor-pointer select-none rounded-lg border border-border px-3 py-2 hover:bg-alt-row/60 transition-colors"
			>
	...
	{:else}
		...
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
			<div class="card p-4 border-l-4 border-l-peligro">
				...
				<p class="text-xs font-semibold uppercase tracking-wide text-text-secondary mt-1">
		...
		<section class="card">
			...
			{#if vencimientosFiltrados.length === 0}
				<p class="text-sm text-text-secondary py-4 text-center">
					Sin vencimientos próximos. <Icon
						name="check"
						class="w-4 h-4 inline-block align-[-2px] text-exito"
					/>
				</p>
			{:else}
						...
						>
							<div class="min-w-0">
								...
								<p class="text-sm text-text-primary mt-1">
									{v.tipo}: <span class="font-bold">{formatDate(v.fecha)}</span>
								</p>
								<p class="text-xs text-text-secondary mt-0.5 truncate">{v.detalle}</p>
							</div>
							<span
								class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold whitespace-nowrap shrink-0 {badgeClases(
									v.critica
								)}"
							>
		...
		<section class="card">
			...
			{#if kmFiltrados.length === 0}
				<p class="text-sm text-text-secondary py-4 text-center">
					Sin mantenimientos próximos por km. <Icon
						name="check"
						class="w-4 h-4 inline-block align-[-2px] text-exito"
					/>
				</p>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
					{#each kmFiltrados as k}
						...
						>
							...
							>
								{k.kmRestante <= 0
									? `${Math.abs(k.kmRestante).toLocaleString('es-CO')} km vencido`
									: `${k.kmRestante.toLocaleString('es-CO')} km`}
							</span>
						</div>
					{/each}
				</div>
		...
		<section class="card">
			...
			{#if rentasFiltradas.length === 0}
				<p class="text-sm text-text-secondary py-4 text-center">
					No hay rentas activas por vencer en los próximos 3 días. <Icon
						name="check"
						class="w-4 h-4 inline-block align-[-2px] text-exito"
					/>
				</p>
							...
							{#each rentasFiltradas as r}
								<tr class="hover:bg-alt-row/50 transition-colors">
									...
									<td class="py-2.5 pr-3 text-text-primary">
										<span class="font-mono text-xs">{r.placa ?? '—'}</span>
										<span class="text-text-secondary text-xs">{r.vehiculo}</span>
									</td>
									<td class="py-2.5 pr-3">
										<span class="{diasClases(diasVencimiento(r.fechaRetorno))} tabular-nums"
											>{formatDate(r.fechaRetorno)}</span
										>
										<p class="text-xs text-text-secondary">
											{diasLabel(diasVencimiento(r.fechaRetorno))}
										</p>
									</td>
									<td class="py-2.5 text-right">
										{#if parseFloat(r.saldoPendiente) > 0}
											<span class="font-bold text-alerta tabular-nums"
												>{formatCOP(r.saldoPendiente)}</span
											>
										{:else}
		...
		<section class="card">
			...
			{#if comparendosFiltrados.length === 0}
				<p class="text-sm text-text-secondary py-4 text-center">
					Sin comparendos pendientes de pago. <Icon
						name="check"
						class="w-4 h-4 inline-block align-[-2px] text-exito"
					/>
				</p>
						...
						<tbody class="divide-y divide-border/60">
							{#each comparendosFiltrados as c}
								<tr class="hover:bg-alt-row/50 transition-colors">
									...
									<td class="py-2.5 pr-3 text-text-primary tabular-nums"
										>{formatDate(c.fechaInfraccion)}</td
									>
									<td class="py-2.5 pr-3 text-text-secondary truncate max-w-[260px]"
										>{c.observaciones ?? '—'}</td
									>
									<td class="py-2.5 text-right font-bold text-alerta tabular-nums"
										>{formatCOP(c.monto)}</td
									>
								</tr>
							{/each}
						</tbody>
