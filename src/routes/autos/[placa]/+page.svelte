		...
		<div class="flex items-center gap-3">
			...
			<div>
				...
				<p class="text-sm text-text-secondary mt-0.5">
					...
					{#if vehiculo}
						· <span
							class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold whitespace-nowrap {estadoRentaClases(
								vehiculo.estado
							)}"
						>
	...
	{:else if timeline}
		...
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
			...
			<div class="card p-4">
				...
				<p class="text-2xl font-bold text-text-primary tabular-nums mt-1">
					{formatCOP(String(timeline.totalPendiente))}
				</p>
			</div>
		</div>
		...
		<div class="card p-5">
			...
			<p class="text-xs text-text-secondary mb-4">
				Rentas (quién tuvo el vehículo y cuándo) y multas en orden cronológico. Cada multa indica
				quién la tenía el día de la infracción.
			</p>
							...
							{#if ev.tipo === 'renta'}
								...
								<div class="rounded-xl border border-primary/20 bg-primary/5 p-4">
									<div class="flex flex-wrap items-center justify-between gap-2">
										...
										<span
											class="inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-semibold whitespace-nowrap {estadoRentaClases(
												r.renta.estado
											)}"
										>
											<span class="w-1.5 h-1.5 rounded-full bg-current opacity-70"></span>
									...
									<p class="text-xs text-text-secondary mt-1">
										...
										{#if r.renta.fechaDevolucionReal}
											<span class="text-text-secondary/70"
												>(devuelto {formatDate(r.renta.fechaDevolucionReal)})</span
											>
										{/if}
									</p>
									<p
										class="text-xs text-text-secondary mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5"
									>
										<span
											>Total <span class="font-semibold text-text-primary tabular-nums"
												>{formatCOP(r.renta.total)}</span
											></span
										>
										{#if parseFloat(r.renta.comision) > 0}
											<span
												>Comisión <span class="font-semibold text-peligro tabular-nums"
													>-{formatCOP(r.renta.comision)}</span
												></span
											>
											<span
												>Valor neto <span class="font-semibold text-text-primary tabular-nums"
													>{formatCOP(r.renta.valorNeto)}</span
												></span
											>
										{/if}
									</p>
							...
							{:else}
								...
								<div class="rounded-xl border border-alerta/25 bg-alerta/5 p-4">
									<div class="flex flex-wrap items-center justify-between gap-2">
										...
										<span
											class="inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-semibold whitespace-nowrap {estadoMultaClases(
												m.estado
											)}"
										>
											<span class="w-1.5 h-1.5 rounded-full bg-current opacity-70"></span>
									...
									{#if m.responsable}
										<p class="text-xs mt-1.5">
											...
											<span class="text-text-secondary">
												(renta {formatContrato(
													m.responsable.anioContrato,
													m.responsable.noContrato
												)})
											</span>
										</p>
									{:else}
