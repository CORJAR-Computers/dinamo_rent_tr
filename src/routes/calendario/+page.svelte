<script lang="ts">
	...
	const idsEnConflicto = $derived(new Set(solapamientos.flatMap((s) => [s.a, s.b])));
	...
	const semanas = $derived(celdasDelMes(mesActual.getFullYear(), mesActual.getMonth()));

	const tituloMes = $derived(nombreMes(mesActual.getFullYear(), mesActual.getMonth()));
	...
	function itemsDelDia(dia: string): { rentas: Renta[]; reservas: Reserva[] } {
		const r = rentas.filter(
			(x) => rangoCubreDia(x.fechaRecogida, x.fechaRetorno, dia) && x.estado !== 'Cancelada'
		);
	...
	<div class="flex flex-wrap items-center justify-between gap-3">
		<div>
			...
			<p class="text-sm text-text-secondary mt-0.5">
				Ocupación mensual por vehículo · {solapamientos.length} conflicto{solapamientos.length === 1
					? ''
					: 's'} de fechas detectado{solapamientos.length === 1 ? '' : 's'}
			</p>
		</div>
					...
					{#each semana as celda}
						...
						>
							<div class="flex items-center justify-between">
								<span
									class="text-xs font-semibold tabular-nums {celda.enMes
										? celda.hoy
											? 'text-primary'
											: 'text-text-primary'
								...
								{#if nDia > 0}
									<span
										class="inline-flex items-center justify-center rounded-full bg-primary/10 text-primary text-[10px] font-bold tabular-nums px-1.5 py-0.5"
									>
										{nDia}
									</span>
								{/if}
								...
								<div class="mt-1 space-y-1">
									{#each r.slice(0, 3) as x}
										<span
											class="block truncate rounded border px-1 py-0.5 text-[10px] font-semibold leading-tight {chipClases(
												'renta',
												idsEnConflicto.has(x.id)
											)}"
											title={`Renta #${x.id} · ${x.nombreCliente}`}
										>
									...
									{#each rs.slice(0, 3) as x}
										<span
											...
											title={`Reserva #${x.id} · ${x.nombreCliente}`}
										>
											Rv{x.id} · {x.nombreCliente.split(' ')[0]}
										</span>
									{/each}
									{#if r.length + rs.length > 3}
										<span class="block text-[10px] text-text-secondary font-medium"
											>+{r.length + rs.length - 3} más</span
										>
									{/if}
								</div>
							{/if}
...
<Modal
	...
	title={detalleDia
		? `Ocupación del ${new Date(detalleDia.dia + 'T00:00:00').toLocaleDateString('es-CO', { weekday: 'long', day: 'numeric', month: 'long' })}`
		: ''}
