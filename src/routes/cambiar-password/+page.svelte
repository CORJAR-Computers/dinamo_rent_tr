...
<div class="min-h-screen flex items-center justify-center bg-fondo p-4 relative overflow-hidden">
	<div
		class="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-primary/10 blur-3xl pointer-events-none"
		aria-hidden="true"
	></div>
	...
	<div class="relative w-full max-w-md">
		<div class="card p-8 shadow-xl">
			...
			>
				...
				<label
					class="flex items-center gap-2 mt-2 mb-4 text-sm text-text-secondary cursor-pointer select-none"
				>
				...
				<div class="mb-5 grid grid-cols-1 gap-1.5">
					{#each passwordHints as hint}
						<div
							...
							class:text-text-secondary={!hint.check()}
						>
							{#if hint.check()}
								<svg
									xmlns="http://www.w3.org/2000/svg"
