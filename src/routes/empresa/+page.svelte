<script lang="ts">
	...
	async function cargar() {
		...
		try {
			...
			error =
				e instanceof ApiError ? e.message : 'No se pudo cargar la configuración de la empresa.';
		} finally {
			cargando = false;
		}
	}
...
<div class="space-y-5 max-w-3xl">
	...
	<div>
		...
		<p class="text-sm text-text-secondary mt-0.5">
			Nombre, datos de contacto y logo. Se usan en el login, el menú lateral, los contratos y las
			órdenes de renta/reserva.
		</p>
	</div>
	...
	{:else}
		{#if error}
			<div
				class="rounded-lg bg-peligro/10 border border-peligro/30 px-3 py-2.5 text-sm text-peligro"
				role="alert"
			>
				{error}
			</div>
		...
		<div class="card p-6">
			...
			<div class="flex items-center gap-5 mb-8">
				<div
					class="w-24 h-24 rounded-2xl bg-white shadow-lg ring-1 ring-border overflow-hidden flex items-center justify-center shrink-0"
				>
					{#if logoDataUrl}
				...
				<div class="flex flex-col gap-2">
					...
					{#if logoDataUrl}
						<button
							class="text-sm text-peligro hover:text-peligro/80 transition-colors inline-flex items-center gap-1.5 w-fit"
							onclick={quitarLogo}
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
					...
					<p class="text-xs text-text-secondary max-w-sm">
						PNG, JPG, WebP o SVG · máximo 2 MB. Se muestra en el login, el menú lateral y las
						impresiones.
					</p>
				</div>
			</div>
			...
			<!-- Datos -->
			<h3 class="text-xs font-bold uppercase tracking-wider text-primary mb-4">
				Datos de la empresa
			</h3>
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
				<div class="sm:col-span-2">
					<FormField
						...
						required
						hint="Aparece en el login, los contratos y las órdenes."
					>
						<input
							class="input"
							placeholder="Ej: DynaRent S.A.S."
				...
				<div class="sm:col-span-2">
					<FormField label="Dirección">
						<input
							class="input"
							placeholder="Ej: Cra 12 # 34-56, Bogotá"
