<script lang="ts">
	...
	async function cargar() {
		...
		loading = true;
		try {
			clientes = await clienteApi.listar(
				sid(),
	...
	function onClienteGuardado(r: ClienteConPii) {
		modalOpen = false;
		toast.success(
			`Cliente ${r.cliente.nombreCompleto} ${clienteEditando ? 'actualizado' : 'creado'}.`
		);
		cargar();
	}
...
<div class="space-y-5">
	...
	<div class="flex flex-wrap items-center justify-between gap-3">
		<div>
			...
			<p class="text-sm text-text-secondary mt-0.5">
				{clientes.length} cliente{clientes.length === 1 ? '' : 's'} registrado{clientes.length === 1
					? ''
					: 's'}
			</p>
		</div>
	...
	{#if piiLegacy}
		<div
			class="rounded-xl border border-alerta/25 bg-alerta/5 px-4 py-3 text-sm text-alerta flex items-start gap-2"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
			...
			>
			<span class="grow">
				Algunos clientes antiguos tienen datos de contacto cifrados (Fernet legacy) que requieren la
				clave original
			...
			{#snippet children(col, item)}
				...
				{:else if col.key === 'licencia'}
					<span class="text-text-secondary text-xs whitespace-nowrap"
						>{licenciaVence(c.vencimientoLicencia)}</span
					>
