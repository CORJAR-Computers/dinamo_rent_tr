...
> **Fecha:** 2026-08-18 · **Estado general:** listo para producción — release v1.0.23 (bumpeada y validada por CI; publicación con el tag), firmada para el auto-update, con backups de la BD (Fase 8: automático, cifrado y restauración), la versión real de la app en el menú/login y la verificación de despliegue -DryRun en el CI.
...
| **Versión estable** | **v1.0.23** — la única release que se distribuye (con auto-update activo) |
...
| **Auto-actualización** | ✅ Activa desde la **v1.0.14** — la app chequea `latest.json` al arrancar y ofrece instalar (firma minisign verificada) | ✅ Secret `TAURI_SIGNING_PRIVATE_KEY` configurado; v1.0.4–v1.0.15 publicadas y firmadas (la v1.0.23 sale firmada con el tag) |

## 2. Releases en GitHub

| Release | Estado | Para quién |
|---|---|---|
| **v1.0.23** | ✅ **Latest / estable** — construida íntegramente por CI (GitHub Actions) | **Única descarga recomendada** |
| v1.0.15 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.23 (a mano, una vez) |
| v1.0.9 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.23 (a mano, una vez) |
| v1.0.3 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.23 (a mano, una vez) |
| v1.0.2 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.23 (a mano; transición al auto-update) |
| v1.0.1 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.23 (a mano) |
| v1.0.0 | ⚠️ **Descontinuada** (prerelease + aviso de deprecación) | Solo referencia — **no instalarla** |

**Assets de la v1.0.23:** [`DinamoRent_1.0.23_x64-setup.exe`](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.0.23/DinamoRent_1.0.23_x64-setup.exe) (NSIS, ~21 MB, recomendado) y [`DinamoRent_1.0.23_x64_en-US.msi`](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.0.23/DinamoRent_1.0.23_x64_en-US.msi) (~33 MB, despliegue GPO; sha256 publicados en la release). La release incluye además los `.sig` y el `latest.json` para el auto-update. Enlaces y credenciales iniciales en [`INSTALACION_OPERACIONES.md`](INSTALACION_OPERACIONES.md).
...
**Actualizar con datos:** idempotente — cada versión abre la BD existente y solo aplica las migraciones pendientes (no hay que desinstalar ni se pierden datos). Cualquier versión anterior → **v1.0.23** (las ≤v1.0.13 se instalan a mano una vez; desde la v1.0.14 las siguientes son automáticas).
...
| **`verificar-despliegue.ps1`** | Post-instalación en el equipo del cliente: exe v1.0.23, arranque vivo 10 s, `config.ini` + BD del primer arranque — veredicto OK/FALLOS | `powershell -File scripts/verificar-despliegue.ps1` |
