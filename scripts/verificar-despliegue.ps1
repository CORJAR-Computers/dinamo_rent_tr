# verificar-despliegue.ps1 - Verificacion post-instalacion de DinamoRent v1.2.0
#
...
#
# Comprueba: exe instalado (v1.2.0), %APPDATA%\com.corjar.dinamorent (config.ini
...
if ($exe) {
    if ($DryRun) {
        $ver = if ($SimularFallo) { '1.0.15.0' } else { '1.2.0.0' }
    } else {
        $ver = (Get-Item $exe).VersionInfo.ProductVersion
    }
    Check "Ejecutable instalado" $true $exe
    Check "Version 1.2.0" ($ver -like '1.2.0*') "ProductVersion=$ver"
} else {
