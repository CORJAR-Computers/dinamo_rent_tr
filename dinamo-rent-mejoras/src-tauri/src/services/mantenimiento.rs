...
impl MantenimientoService {
    ...
    ///
    /// TRANSACCIÓN: INSERT mantenimiento + (si es cambio de aceite) UPDATE
    /// autos.proximo_aceite. Atómico: si la sincronización de proximo_aceite
    /// falla, el INSERT se revierte y no queda mantenimiento huérfano con el
    /// kilometraje del vehículo desactualizado. (Fix bug #2 del roadmap.)
    pub fn crear(
        conn: &mut PooledConnection,
        cfg: &Arc<AppConfig>,
        ...
        // Pre-calcular valores que pueden fallar (AppError) antes de la tx
        let fecha = parse_fecha(&datos.fecha)?;
        let es_cambio_aceite = datos.tipo.trim().to_uppercase()
            == TIPO_CAMBIO_ACEITE.to_uppercase();
        ...
        let id: i64 = conn
            .with_transaction(|tx| -> Result<i64, rsfbclient::FbError> {
                ...
                let (id,): (i64,) = tx.execute_returnable(
                    ...
                    params![
                        datos.placa.to_string(),
                        datos.tipo.to_string(),
                        fecha,
                        opt_str(&datos.descripcion),
                        opt_str(&datos.observaciones),
                        datos.costo.to_string(),
                        datos.km_proximo_cambio_aceite,
                        datos.costo.to_string(),
                    ],
                )?;
                // 2) Si es cambio de aceite, sincronizar autos.proximo_aceite
                if es_cambio_aceite {
                    tx.execute(
    ...
    ///
    /// TRANSACCIÓN: UPDATE mantenimiento + (si es cambio de aceite) UPDATE
    /// autos.proximo_aceite. Atómico: si la sincronización falla, el UPDATE
    /// del mantenimiento se revierte. (Fix bug #2 del roadmap.)
    pub fn actualizar(
        conn: &mut PooledConnection,
        cfg: &Arc<AppConfig>,
        id: i64,
